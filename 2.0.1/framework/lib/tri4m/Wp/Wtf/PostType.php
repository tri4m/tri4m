	...
	CLASS PostType EXTENDS \tri4m\Wp\Wtf
	{
		...
		function __construct(__type_Posttype $__Setup)
		{
			...
			$this->__actions[__const_Action::INIT] = new __type_Action([
				...
				{
					if(FALSE !== Inv::postTypeExists($this->__Setup->name->slugSingular))
						return;
					...
					foreach([
						...
						$config['labels'][$k] = $this->__Setup->env->labels->get()->toArray()[$i];
					...
					foreach([
						...
						$config['rewrite'][$k] = $this->__Setup->env->rewrite->get()->toArray()[$i];
					...
					Inv::registerPostType($this->__Setup->name->slugSingular, $config);
				}
			]);
			
			$this->__filters[__const_Filter::POST_UPDATED_MESSAGES] = new __type_Filter([
				__type_Filter::argsNum	=> 1,
				__type_Filter::fn	=> function($__messages) use (&$__name)
				{
					$post	= Inv::glob('post');
					$id	= Inv::glob('post_ID');
					$x	= 0;
					$h	= &$__messages[$this->__Setup->name->slugSingular];
					$p	= $__name +
					[
					...
					foreach([
						...
						$h[++$x] = String::insert($this->__Setup->updateMsg->get()->toArray()[$i], $p);
